...
slider1.noUiSlider.on("slide", function(){
    ...
    for(let i=0; i<numbOfSteps+1; i++){
        load1[i] = {x:Number(slideMin)+2*i, y:Number(y1)}; //[ton]
    }
...
slider2.noUiSlider.on("slide", function(){
    ...
    load2.length = 0;
    for(let i=0; i<numbOfSteps+1; i++){
        load2[i] = {x:Number(slideMin)+2*i, y:Number(y2)}; // [ton]
    }
...
function getInputValue(count){
    ...
    for(let i=0; i<numbOfSteps+1; i++){
        this["load"+count][i] = {x:Number(slideMin)+2*i, y:Number(this["y"+count])}; 
        }
...
function stillWaterShearForce(){
    ...
    shearForce[0] = { x:0 , y:0 };
    for (let i=1; i<Math.ceil(ship.L/2)+1; i++){
        
//        console.log(i);
//         console.log(shearForce[i-1]);
//         console.log(loadsResultant[i-1]);
...
function stillWaterBendingMoment(){
    ...
    for (let i=1; i<Math.ceil(ship.L/2)+1; i++){
        ...
        stillWaterBM.push({ x:i*2 , y:BM }); //[kNm]
    }
}
